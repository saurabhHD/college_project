...
<html>
	...
	<body>
		...
		<div class="container">
			...
			<div class="row">
				<?php
				$mybook_query = "SELECT * FROM products WHERE seller_id='$user_id' AND status='stock'";
				$mybook_response = $db->query($mybook_query);
				if($mybook_response)
				{
					while($mybook_data = $mybook_response->fetch_assoc())
					{
						...
						<div class="main-box d-flex justify-content-center">
							<div class="content" style="border-radius: 15px">
								...
								<div class="text-box">
									...
									<button class="btn font-weight-bold remove-btn rounded-lg" product-id="'.base64_encode($mybook_data['id']).'">Remove</button>
									</div>
								</div>
							</div>
						</div>
			...
			$(document).ready(function(){
				$(".remove-btn").each(function(){
					$(this).click(function(){
						...
						$("#remove-product-alert").modal('show');
						$(".yes-btn").click(function(){
							$("#remove-product-alert").modal('hide');
							$.ajax({
								type : "POST",
								url : "../php/remove_product.php",
								data : {
								...
								success : function(response)
								{
									...
									{
										var div = remove_btn.parentElement.parentElement.parentElement.parentElement.parentElement;
										div.remove();
									}
								}
							});
